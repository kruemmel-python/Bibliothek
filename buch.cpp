...
// Funktion zur Rückgabe eines Buches
...
void buchHinzufuegen(std::vector<Buch>& sammlung, const std::string& titel, const std::string& autor, int id) {
    // Prüfen, ob ein Buch mit der gleichen ID bereits in der Sammlung existiert
    ...
    // Wenn kein Buch mit der ID existiert, hinzufügen
    Buch neuesBuch;
    buchBeschreiben(neuesBuch, titel, autor, id);
    sammlung.push_back(neuesBuch);
    std::cout << "Das Buch '" << titel << "' wurde erfolgreich zur Sammlung hinzugefügt.\n";
}
